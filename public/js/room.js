...
async function screenShot() {
    ...
    const imageData = canvas.toDataURL();
      ...
      // Pass the image data to the extractText function
      await extractText(imageData);
    } catch (error) {
      console.error('Error capturing screen and extracting text:', error);
      alert('Error capturing screen and extracting text');
    }
  }
...
async function extractText(image) {
    ...
    if (!image) {
      ...
      try {
        const response = await fetch('http://172.25.54.24:5000/upload', {
          method: 'POST',
          headers: {
        ...
        const extractedText = data.text[0].generated_text;
